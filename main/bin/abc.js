...
let runBuild = () => {
  inquirer.prompt({
    ...
    if (answer.chioce == 'test') {
      cmdStr = 'cross-env BUILD_ENV=test npm run build'
      envStr = '测试环境'
    } else if (answer.chioce == 'production') {
      cmdStr = 'cross-env BUILD_ENV=production npm run build'
      envStr = '正式环境'
    }
...
let runDev = () => {
  inquirer.prompt({
    ...
    let cmdStr
    if (answer.chioce == 'test') {
      cmdStr = 'cross-env BUILD_ENV=test npm run dev'
    } else if (answer.chioce == 'production') {
      cmdStr = 'cross-env BUILD_ENV=production npm run dev'
    }
